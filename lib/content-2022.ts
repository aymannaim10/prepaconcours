...
export const RECAP_2022: RecapTopic[] = [
  ...
  },
  {
    ...
    formulas: [
      ...
      { label: 'Discriminant', latex: `\\Delta = a^{2} - 4b`, description: 'Compute the discriminant of the characteristic quadratic to choose the right case below.' },
      { label: 'Two real roots ($\\Delta > 0$)', latex: `r_{1} = \\dfrac{-a - \\sqrt{\\Delta}}{2} \\;,\\; r_{2} = \\dfrac{-a + \\sqrt{\\Delta}}{2} \\implies y = A\\,e^{r_{1}x} + B\\,e^{r_{2}x}`, description: 'Discriminant strictly positive. Two distinct real roots — general solution is sum of two exponentials.' },
      { label: 'Double root ($\\Delta = 0$)', latex: `r = \\dfrac{-a}{2} \\implies y = (A + B\\,x)\\,e^{r x}`, description: 'Discriminant zero. Repeated root — multiply one basis solution by $x$ to get a second linearly independent one.' },
      { label: 'Complex roots ($\\Delta < 0$)', latex: `r = \\alpha \\pm i\\beta \\;,\\; \\alpha = \\dfrac{-a}{2} \\;,\\; \\beta = \\dfrac{\\sqrt{-\\Delta}}{2} \\implies y = e^{\\alpha x}\\bigl(A\\cos(\\beta x) + B\\sin(\\beta x)\\bigr)`, description: 'Discriminant strictly negative. Conjugate complex roots — sinusoidal oscillation modulated by an exponential envelope.' },
    ...
    theorems: [
      { name: 'Decision tree by discriminant', statement: `\\Delta = a^{2} - 4b \\implies \\begin{cases} \\Delta > 0: y = A e^{r_{1}x} + B e^{r_{2}x},\\;\\; r_{1,2} = \\dfrac{-a \\pm \\sqrt{\\Delta}}{2} \\\\ \\Delta = 0: y = (A + Bx) e^{r x},\\;\\; r = \\dfrac{-a}{2} \\\\ \\Delta < 0: y = e^{\\alpha x}(A\\cos\\beta x + B\\sin\\beta x),\\;\\; \\alpha = \\dfrac{-a}{2},\\,\\beta = \\dfrac{\\sqrt{-\\Delta}}{2} \\end{cases}`, keyIdea: 'Three-row table indexed by the SIGN of $\\Delta$. Memorize this — it covers every 2nd order linear homogeneous ODE.' },
